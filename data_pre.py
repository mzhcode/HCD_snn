...
def load_dataset(Dataset):
    ...
    if Dataset == "FM":
        data_b = sio.loadmat('G://GTMSiam-main//farmland//farm06.mat')
        data_a = sio.loadmat('G://GTMSiam-main//farmland//farm07.mat')
        data_before = data_b['imgh']
        data_after = data_a['imghl']
        gt_mat = sio.loadmat('G://GTMSiam-main//farmland//label.mat')
        gt = gt_mat['label']
        gt = 2 - gt
        dataset_name = "farm"
    if Dataset == "RV":
        data_b = sio.loadmat('G://GTMSiam-main//data//river//river_before.mat')
        data_a = sio.loadmat('G://GTMSiam-main//data//river//river_after.mat')
        data_before = data_b['river_before']
        data_after = data_a['river_after']
        gt_mat = sio.loadmat('G://GTMSiam-main//data//river//groundtruth.mat')
        gt = gt_mat['lakelabel_v1']
        dataset_name = "river"
        gt = np.where(gt == 255, 1, gt)
        gt = 2 - gt
    if Dataset == "hermiston":
        data_b = sio.loadmat('/home/think/spikehcd_test/data/hermiston/Sa1.mat')
        data_a = sio.loadmat('/home/think/spikehcd_test/data/hermiston/Sa2.mat')
        data_before = data_b['T1']
        data_after = data_a['T2']
        gt_mat = sio.loadmat('/home/think/spikehcd_test/data/hermiston/SaGT.mat')
        gt = gt_mat['GT']
    ...
    # X1 = data_before
    # X2 = data_after
    return X1, X2, gt, dataset_name

def select_rect_no_pad(hsi1, hsi2, target_coords, pitch_size = 11):
...
def  Grammar(img1, img2, tgt_coords, method = "rect 11"):
    try:
    ...
    if region_type == "rect":
        ...
        data1, data2 = select_rect_no_pad(img1, img2, tgt_coords, pitch_size=param)
    elif region_type == "dot":
        data = select_rect_no_pad(img1, tgt_coords, pitch_size = 1)
        data = np.reshape(data, [data.shape[0], data.shape[1] * data.shape[2], data.shape[3]])
    return data1, data2
def zeropad_to_max_len(data, max_len = 25):
...
def sampling(sampling_mode, train_rate, gt):
    ...
    if sampling_mode == 'random':
        ...
        val_count = int(0.02 * (len(test_index) + len(train_index)))
