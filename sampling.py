...
def get_train_test(data, data_labels, val_size, test_size = None, shuffle = True): 
    ...
    for index, label in enumerate(unique_labels):
        ...
        if test_size:       
            ...
            remaining_indexes = list(set(range(length)) - set(test_indexes))
            val_indexes =  random.sample(remaining_indexes, nb_val)
            train_indexes = np.array([i for i in range(length) if i not in test_indexes and i not in val_indexes])
